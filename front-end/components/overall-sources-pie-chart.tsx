...
export function OverallSourcesPieChart() {
  ...
  // Filtrar para mostrar apenas fontes com valores
  const filteredData = useMemo(() => chartData.filter((item) => item.value > 0), [chartData])

  if (filteredData.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-muted-foreground">Não há receitas registradas para visualizar no gráfico.</p>
      </div>
    )
  }
